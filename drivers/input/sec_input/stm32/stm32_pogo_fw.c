...
static int stm32_dev_firmware_update(struct stm32_dev *data, const u8 *fw_data, size_t size, int retry)
{
	...
	/*
	 * ubuntu-tab bring-up safety guard: disable the MCU flash erase/program
	 * path. The cover ships with working stock firmware; for first-light we
	 * only want to prove bind + power + attach + key events without any risk
	 * of a spurious reflash (e.g. a garbled first-contact version/CRC read
	 * being misread as "needs upgrade"). Returning success here leaves the
	 * MCU flash untouched. Define STM32_POGO_ENABLE_FW_WRITE to restore real
	 * firmware updates once first-light is confirmed.
	 */
#ifndef STM32_POGO_ENABLE_FW_WRITE
	input_info(true, &data->client->dev,
		"%s: MCU firmware write DISABLED for bring-up; skipping erase/program\n",
		__func__);
	return 0;
#endif
