 ...
 *
 * Two staged brown-out protections the part performs autonomously, both of
 * which this device's own stock firmware programs on all four amplifiers and
 * neither of which mainline configured before. BPE watches the battery rail VP
 * against four thresholds and applies a per-level attenuation with its own
 * attack/hold/release; boost-BPE drives the boost converter's current limits
 * off the same ladder, and the IL_LIM block adds an inductor-current limiter.
 *
 * Register and field definitions transcribed MECHANICALLY from the device's own
 * downstream driver (cs35l45.h, SM-X910 kernel source) rather than retyped -- a
 * transposed digit in a mask is invisible in review and silently writes the
 * wrong field.
 */
...
#define CS35L45_BBPE_EN_SHIFT			13
#define CS35L45_ABPE_EN_SHIFT			12
...
struct cs35l45_private {
	...
	bool initialized;
	/*
	 * Set when the device tree carries a "cirrus,classh-config" node, so
	 * that the CH_OVB_LATCH pulse is re-issued after regcache_sync() on a
